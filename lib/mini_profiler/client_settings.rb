  ...
  class MiniProfiler
    class ClientSettings
      ...
      def initialize(env, store, start)
        ...
        if @cookie
          @cookie
            ...
            .map { |pair| pair.split('=') }
            .each do |k, v|
              @orig_disable_profiling = @disable_profiling = (v == 't') if k ==
                'dp'
        ...
        if !@backtrace_level.nil? &&
             (@backtrace_level == 0 || @backtrace_level > BACKTRACE_NONE)
          @backtrace_level = nil
        end
      ...
      def handle_cookie(result)
        ...
        if (
             MiniProfiler.config.authorization_mode == :whitelist &&
      ...
      def write!(headers)
        ...
        if MiniProfiler.request_authorized? &&
             MiniProfiler.config.authorization_mode == :whitelist
        ...
        if @orig_disable_profiling != @disable_profiling ||
             @orig_backtrace_level != @backtrace_level || @cookie.nil? ||
             tokens_changed
          settings = { 'p' => 't' }
          settings['dp'] = 't' if @disable_profiling
          settings['bt'] = @backtrace_level if @backtrace_level
          settings['a'] = @allowed_tokens.join('|') if @allowed_tokens &&
            MiniProfiler.request_authorized?
      ...
      def has_valid_cookie?
        ...
        if (MiniProfiler.config.authorization_mode == :whitelist) &&
          ...
          valid_cookie =
            @allowed_tokens && (Array === @orig_auth_tokens) &&
              ((@allowed_tokens & @orig_auth_tokens).length > 0)
        end
