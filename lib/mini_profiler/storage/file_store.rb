...
module Rack
  class MiniProfiler
    class FileStore < AbstractStore
      ...
      class CacheCleanupThread < Thread; end
      ...
      def set_all_unviewed(user, ids)
        @user_view_lock.synchronize { @user_view_cache[user] = ids.uniq }
      end
      ...
      def flush_tokens
        @auth_token_lock.synchronize { @auth_token_cache[''] = nil }
      end

      def allowed_tokens
        @auth_token_lock.synchronize do
          token1, token2, cycle_at = @auth_token_cache['']

          unless cycle_at && (Float === cycle_at) &&
      ...
      def cleanup_cache
        ...
        @timer_struct_lock.synchronize do
          files.each do |f|
            f = @path + '/' + f
            if ::File.basename(f) =~ (/^mp_timers/) &&
                 ((Time.now - ::File.mtime(f)) > @expires_in_seconds)
              ::File.delete f
            end
          end
        end
