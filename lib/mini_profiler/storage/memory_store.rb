  ...
  class MiniProfiler
    class MemoryStore < AbstractStore
      ...
      def initialize_cleanup_thread(args = {})
        ...
        t =
          CacheCleanupThread.new(cleanup_interval, cleanup_cycle, self) do
            t.sleepy_run until Thread.current[:should_exit]
          end
        at_exit { t[:should_exit] = true }
      end

      def save(page_struct)
        @timer_struct_lock.synchronize do
          @timer_struct_cache[page_struct[:id]] = page_struct
        end
      end

      def load(id)
        @timer_struct_lock.synchronize { @timer_struct_cache[id] }
      end

      def set_unviewed(user, id)
        @user_view_lock.synchronize do
      ...
      def set_all_unviewed(user, ids)
        @user_view_lock.synchronize { @user_view_cache[user] = ids }
      end

      def get_unviewed_ids(user)
        @user_view_lock.synchronize { @user_view_cache[user] }
      end
      ...
      def cleanup_cache
        ...
        @timer_struct_lock.synchronize do
          @timer_struct_cache.delete_if do |k, v|
            v[:started] < expire_older_than
          end
        end
      end
      ...
      def allowed_tokens
        @token_lock.synchronize do
          unless @cycle_at &&
            ...
            @cycle_at =
              Process.clock_gettime(Process::CLOCK_MONOTONIC) +
                Rack::MiniProfiler::AbstractStore::MAX_TOKEN_AGE
          end
      ...
      def push_snapshot(page_struct, config)
        @snapshots_lock.synchronize do
          ...
          @snapshots.reverse!
          @snapshots.slice!(-1) if @snapshots.size > config.snapshots_limit
        end
      end
      ...
      def load_snapshot(id)
        @snapshots_lock.synchronize { @snapshots.find { |s| s[:id] == id } }
      end
