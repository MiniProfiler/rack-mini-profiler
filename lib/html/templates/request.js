<div class="profiler-result">
  ...
  <div class="profiler-popup">
    <div class="profiler-info">
      ...
      <span class="profiler-server-time">{{machine_name}} on {{started_date}}</span>
    </div>
    <div class="profiler-output">
      <table class="profiler-timings">
        <thead>
          ...
          {{/if}}
          {{#each formatted_custom_timing_names}}
            <th colspan="2">{{ this }} (ms)</th>
          {{/each}}
          </tr>
        </thead>
        <tbody>
      ...
      <table class="profiler-timings profiler-client-timings">
        ...
        <tbody>
          {{#each formatted_client_timings}}
          <tr class="{{#if this.isTrivial }}profiler-trivial{{/if}}">
            <td class="profiler-label">{{this.name}}</td>
            <td class="profiler-duration">
              {{#if this.durationOverZero }}
                <span class="profiler-unit"></span>{{this.duration}}
              {{/if}}
            </td>
            <td class="profiler-duration time-from-start">
              <span class="profiler-unit">+</span>{{this.start}}
            </td>
          </tr>
          {{/each}}
        </tbody>
