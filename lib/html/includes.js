...
var MiniProfiler = (function () {
    ...
    return {
        ...
        getWebServiceTimings: function (root) {
            var result = [],
                addToResults = function (timing) {
                    if (timing.WebServiceTimings) {
                        for (var i = 0, webServiceTiming; i < timing.WebServiceTimings.length; i++) {
                            webServiceTiming = timing.WebServiceTimings[i];

                            // HACK: add info about the parent Timing to each WebServiceTiming so UI can render
                            webServiceTiming.ParentTimingName = timing.Name;
                            result.push(webServiceTiming);
                        }
                    }
        ...
        getWebServiceTimingsCount: function (root) {
            var result = 0,
                countWebService = function (timing) {
                    if (timing.WebServiceTimings) {
                        result += timing.WebServiceTimings.length;
                    }

                    if (timing.Children) {
                        for (var i = 0; i < timing.Children.length; i++) {
                            countWebService(timing.Children[i]);
                        }
                    }
                };
            countWebService(root);
            return result;
        },
