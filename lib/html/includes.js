...
var MiniProfiler = (function() {
  ...
  var save = function(keyPrefix, value) {
    ...
    localStorage[getVerseonedKey(keyPrefix)] = value;
  };
    ...
    for (i = 0; i < ids.length; i++) {
      ...
      if (id == options.currentId) {
        ...
        if (clientPerformance !== null) {
          ...
          var timing = clientPerformance.timing;
          for (p in timing) {
            if (!(typeof timing[p] === "function")) {
      ...
      if (fetchedIds.indexOf(id) < 0 && fetchingIds.indexOf(id) < 0) {
        ...
        (function() {
          ...
          var params = `id=${id}&clientPerformance=${JSON.stringify(clientPerformance)}&clientProbes=${clientProbes}&popup=1`;
  ...
  var renderTemplate = function(json) {
    if (Handlebars.partials.length == 0)
      Handlebars.partials = Handlebars.templates;
    json['duration'] = MiniProfiler.formatDuration(json['duration_milliseconds'])
    json['show_total_sql_count'] = MiniProfiler.showTotalSqlCount()
    json['started_date'] = MiniProfiler.renderDate(json['started'])
    json['hide_client_timings'] = !json['client_timings']
    json['formatted_client_timings'] = MiniProfiler.formatClientTimings(json['client_timings'])
    json['formatted_custom_timing_names'] = json['custom_timing_names'].map(name => name.toLowerCase())
    json['percent_in_sql'] = MiniProfiler.formatDuration(json['duration_milliseconds_in_sql'] / json['duration_milliseconds'] * 100)
    console.log(json)
    var request = Handlebars.templates["request.js"](json)
    var node = document.createElement('div')
    node.innerHTML = request
    console.log(node.children[0])
    return [node.children[0]]
    // return document.getElementById('profilerTemplate').append(request)
    // return $("#profilerTemplate").tmpl(json);
  };

  var buttonShow = function(json) {
    var result = renderTemplate(json)[0];
  ...
  var toggleHidden = popup => {
    ...
    [childrenTime, trivialGaps].forEach(el =>
      el.addEventListener("click", function() {
        toggleIt(this);
      })
    );
    ...
    // if (options.showTrivial || trivial.getAttribute("show-on-load")) {
      // toggleIt(trivial);
    // }
  ...
  var queriesScrollIntoView = (link, queries, whatToScroll) => {
    ...
    whatToScroll.scrollTop = whatToScroll.scrollTop + cells[0].offsetTop - 100;
  ...
  var bindDocumentEvents = function() {
    ...
    Mousetrap.bind(options.toggleShortcut, keydownEvent);
  ...
  return {
    init: function() {
      ...
      if (major === 3 || major === 2 || (major === 1 && minor >= 7)) {
        ...
        load(options.path + "jquery.3.4.1.js?v=" + options.version, function() {
      ...
      if (typeof Mousetrap !== "function")
        load(options.path + "mousetrap.js?v=" + options.version, () => {});
      if (typeof Handlebars !== "function")
        load(options.path + "templates/handlebars-runtime.js?v=" + options.version, () => {
          // Load all compiled templates
          load(options.path + "templates/compiled/request.js?v=" + options.version, () => {});
        });
    },
    ...
    formatClientTimings: function(clientTimings) {
      var list = [];
      var t;
      if (!clientTimings || !clientTimings.timings) return [];
      ...
      for (var i = 0; i < clientTimings.timings.length; i++) {
        ...
        list.push({
          isTrivial: trivial,
          name: t.Name,
          duration: MiniProfiler.formatDuration(t.Duration),
          durationOverZero: (t.Duration > 0),
          start: MiniProfiler.formatDuration(t.Start)
        });
      }
      ...
      if (window.performance && window.performance.getEntriesByName) {
        ...
        if (firstPaint !== undefined && firstPaint.length >= 1) {
          list.push({
            ...
            duration: MiniProfiler.formatDuration(firstPaint[0].duration),
            durationOverZero: (firstPaint[0].duration > 0),
            start: MiniProfiler.formatDuration(firstPaint[0].startTime)
          });
        }
      }
    ...
    showTotalSqlCount: function(count) {
      return count;
    }
  };
})();
