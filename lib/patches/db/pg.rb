...
class PG::Result
  ...
  def values(*args, &blk)
    unless defined?(@miniprofiler_sql_id)
      return values_without_profiling(*args, &blk)
    end
    mp_report_sql { values_without_profiling(*args, &blk) }
  end
  ...
  def each(*args, &blk)
    ...
    mp_report_sql { each_without_profiling(*args, &blk) }
  end
...
class PG::Connection
  ...
  def prepare(*args, &blk)
    ...
    unless SqlPatches.should_measure?
      return prepare_without_profiling(*args, &blk)
    end
  ...
  if Gem::Version.new(PG::VERSION) >= Gem::Version.new('1.1.0')
    def exec_params(*args, &blk)
      unless SqlPatches.should_measure?
        return exec_params_without_profiling(*args, &blk)
      end
  ...
  def exec_prepared(*args, &blk)
    unless SqlPatches.should_measure?
      return exec_prepared_without_profiling(*args, &blk)
    end
  ...
  def send_query_prepared(*args, &blk)
    unless SqlPatches.should_measure?
      return send_query_prepared_without_profiling(*args, &blk)
    end
  ...
  def async_exec(*args, &blk)
    unless SqlPatches.should_measure?
      return async_exec_without_profiling(*args, &blk)
    end
