  ...
  class << self
    ...
    def get_all(client, fetch_list)
      unless SqlPatches.should_measure?
        return get_all_without_profiling(client, fetch_list)
      end
...
class Riak::Client
  ...
  def delete_object(bucket, key, options = {})
    profile(
      "delete_object bucket=#{bucket.name} key=#{key} options=#{options}"
  ...
  def get_bucket_props(bucket, options = {})
    profile("get_bucket_props bucket=#{bucket.name} options=#{options}") do
  ...
  def get_index(bucket, index, query, options = {})
    profile(
      "get_index bucket=#{bucket.name} index=#{index} query=#{query} options=#{
        options
      }"
  ...
  def get_preflist(bucket, key, type = nil, options = {})
    profile(
      "get_preflist bucket=#{bucket.name} key=#{key} type=#{type} options=#{
        options
      }"
    ) { get_preflist_without_profiling(bucket, key, type, options) }
  end

  alias_method :get_object_without_profiling, :get_object
  def get_object(bucket, key, options = {})
    profile("get_object bucket=#{bucket.name} key=#{key} options=#{options}") do
  ...
  def list_keys(bucket, options = {}, &block)
    profile("list_keys bucket=#{bucket.name} options=#{options}") do
  ...
  def reload_object(object, options = {})
    profile(
      "reload_object bucket=#{object.bucket.name} key=#{object.key} vclock=#{
        object.vclock
      } options=#{options}"
  ...
  def set_bucket_props(bucket, properties, type = nil)
    profile("set_bucket_props bucket=#{bucket.name} type=#{type}") do
      set_bucket_props_without_profiling(bucket, properties, type)
    end
  end

  alias_method :clear_bucket_props_without_profiling, :clear_bucket_props
  def clear_bucket_props(bucket, options = {})
    profile("clear_bucket_props bucket=#{bucket.name} options=#{options}") do
  ...
  def store_object(object, options = {})
    profile(
      "store_object bucket=#{object.bucket.name} key=#{object.key} vclock=#{
        object.vclock
      } options=#{options}"
