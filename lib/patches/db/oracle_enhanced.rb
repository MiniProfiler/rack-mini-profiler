...
class ActiveRecord::ConnectionAdapters::OracleEnhancedAdapter
  ...
  def exec_insert(sql, name, binds, pk = nil, sequence_name = nil)
    mp_profile_sql(sql, name) do
      exec_insert_without_profiling(sql, name, binds, pk, sequence_name)
    end
  end
  ...
  def mp_profile_sql(sql, name, &blk)
    ...
    if (result && !result.frozen?)
      result.instance_variable_set('@miniprofiler_sql_id', record)
    end
  ...
  def mp_should_measure?(name)
    ...
    !(
      Rack::MiniProfiler.config.skip_schema_queries &&
        SCHEMA_QUERY_TYPES.include?(name)
    )
  end
end
