...
module Rack::MiniProfilerRailsMethods
  def render_notification_handler(
    ...
    node[:duration_without_children_milliseconds] =
      duration_ms - children_duration
    ...
    current.custom_timings.each do |type, timings|
      ...
      to_be_moved[:custom].each do |custom|
        current.move_custom(type, custom, node)
      end
    end
  end
  ...
  def get_webpacker_assets_path
    if defined?(Webpacker) && Webpacker.config.config_path.exist?
      Webpacker
        ...
        .public_output_path
        .to_s
        .gsub(Webpacker.config.public_path.to_s, '')
    end
  end
