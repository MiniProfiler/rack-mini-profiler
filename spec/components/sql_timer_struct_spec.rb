...
describe Rack::MiniProfiler::SqlTimerStruct do
  ...
  describe 'valid sql timer' do
    ...
    it 'has an execute_type' do
      @sql[:execute_type].should_not be_nil
    end

    it 'has a formatted_command_string' do
      @sql[:formatted_command_string].should_not be_nil
    end

    it 'has a stack_trace_snippet' do
      @sql[:stack_trace_snippet].should_not be_nil
    end

    it 'has a start_milliseconds' do
      @sql[:start_milliseconds].should_not be_nil
    end

    it 'has a duration_milliseconds' do
      @sql[:duration_milliseconds].should_not be_nil
    end

    it 'has a is_duplicate' do
      @sql[:is_duplicate].should_not be_nil
    end
  end
