    ...
    @app ||=
      Rack::Builder.new do
        ...
        map '/no_exceptions' do
          run lambda { |_env|
                [200, { 'Content-Type' => 'text/html' }, '<h1>Success</h1>']
              }
        end
        map '/raise_exceptions' do
          ...
          run lambda { |_env|
                ...
                [
                  ...
                  '<h1>Exception raised but success returned</h1>'
                ]
              }
        end
      end.to_app
  ...
  describe 'with exceptions' do
    it 'unfiltered' do
      ...
      expect(last_response.body).to include(
        'RuntimeError - "Test RuntimeError Exception"'
      )
      expect(last_response.body).to include(
        'NameError - "Test NameError Exception"'
      )
      expect(last_response.body).to include(
        'NoMethodError - "Test NoMethodError Exception"'
      )
    ...
    it 'with a single filtered exception' do
      get '/raise_exceptions',
          :pp => 'trace-exceptions',
          'trace_exceptions_filter' => 'RuntimeError'
    ...
    it 'with a multiple filtered exceptions' do
      get '/raise_exceptions',
          :pp => 'trace-exceptions',
          'trace_exceptions_filter' => 'NameError|NoMethodError'
