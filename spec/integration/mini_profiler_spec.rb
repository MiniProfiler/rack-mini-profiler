...
describe Rack::MiniProfiler do
  ...
  def app
    @app ||=
      Rack::Builder.new do
        ...
        map '/path1/a' do
          run lambda { |env|
                [200, { 'Content-Type' => 'text/html' }, +'<h1>path2</h1>']
              }
        end
        ...
        map '/post' do
          run lambda { |env|
                [302, { 'Content-Type' => 'text/html' }, +'<h1>POST</h1>']
              }
        end
        map '/html' do
          run lambda { |env|
        ...
        map '/under_passenger' do
          run lambda { |env|
                [
                  200,
                  { 'Content-Type' => 'text/html' },
                  +'<html><h1>and I ride and I ride</h1></html>'
                ]
              }
        end
        map '/create' do
        ...
        map '/notallowed' do
          run lambda { |env|
                [
                  403,
                  { 'Content-Type' => 'text/html' },
                  +'<html><h1>you are not allowed here</h1></html>'
                ]
              }
        end
        map '/whoopsie-daisy' do
          run lambda { |env|
                [
                  500,
                  { 'Content-Type' => 'text/html' },
                  +'<html><h1>whoopsie daisy</h1></html>'
                ]
              }
        end
        map '/test-snapshots-custom-fields' do
          run lambda { |env|
                ...
                qp.each { |k, v|
                  Rack::MiniProfiler.add_snapshot_custom_field(k, v)
                }
                [
                  200,
                  { 'Content-Type' => 'text/html' },
                  +'<html><h1>custom fields</h1></html>'
                ]
              }
        end
      end.to_app
  ...
  before { Rack::MiniProfiler.reset_config }
  ...
  describe 'with a valid request' do
    ...
    it 'has the JS in the body' do
      expect(
        last_response.body.include?('/mini-profiler-resources/includes.js')
      ).to be(true)
    end
  ...
  describe 'with a SCRIPT_NAME' do
    before { get '/html', nil, 'SCRIPT_NAME' => '/test' }

    it 'has the JS in the body with the correct path' do
      expect(
        last_response.body.include?('/test/mini-profiler-resources/includes.js')
      ).to be(true)
    end
  end

  describe 'within a rails engine' do
    before { get '/rails_engine' }

    it 'include the correct JS in the body' do
      expect(
        last_response.body.include?(
          '/rails_engine/mini-profiler-resources/includes.js'
        )
      ).not_to be(true)
      expect(
        last_response.body.include?('src="/mini-profiler-resources/includes.js')
      ).to be(true)
    end
  end

  describe 'under passenger' do
    before { ENV['PASSENGER_BASE_URI'] = '/under_passenger' }

    after { ENV['PASSENGER_BASE_URI'] = nil }

    it 'include the correct JS in the body' do
      get '/under_passenger'
      expect(
        last_response.body.include?(
          'src="/under_passenger/mini-profiler-resources/includes.js'
        )
      ).to be(true)
    end
  end
  ...
  describe 'configuration' do
    it 'should remove caching headers by default' do
      get '/cached-resource'
      expect(
        last_response.headers['X-MiniProfiler-Original-Cache-Control']
      ).to eq('original-cache-control')
    ...
    describe 'with caching re-enabled' do
      ...
      it 'should be able to re-enable caching' do
        ...
        expect(last_response.headers['Cache-Control']).not_to include(
          'no-store'
        )
      end
    end
    ...
    it 'skips paths listed' do
      Rack::MiniProfiler.config.skip_paths = %w[/path/ /path2/]
      get '/path2/a'
      expect(last_response.headers.has_key?('X-MiniProfiler-Ids')).to be(false)
      get '/path1/a'
      expect(last_response.headers.has_key?('X-MiniProfiler-Ids')).to be(true)
  ...
  describe 'authorization mode whitelist' do
    before { Rack::MiniProfiler.config.authorization_mode = :whitelist }
  ...
  describe 'error handling when storage_instance fails to save' do
    it 'should recover gracefully' do
      ...
      allow_any_instance_of(Rack::MiniProfiler::MemoryStore).to receive(:save) {
        raise 'This error'
      }
  ...
  context 'snapshots sampling' do
    ...
    after(:each) { Rack::MiniProfiler.reset_config }
    ...
    it 'does not take snapshots of paths in skip_paths' do
      ...
      get '/path2/a'
      expect(
        Rack::MiniProfiler.config.storage_instance.snapshot_groups_overview
      ).to eq([])
    end
    ...
    it 'custom fields are reset between requests' do
      ...
      id2 =
        store.find_snapshots_group(group_name).find { |s| s[:id] != id1 }[:id]
  ...
  context 'snapshots page' do
    it 'allows only authorized users to access it' do
      ...
      expect(last_response.body).to eq(
        'Not Found: /mini-profiler-resources/snapshots'
      )
    ...
    it 'without group_name param it sends groups list in response' do
      ...
      struct =
        Rack::MiniProfiler::TimerStruct::Page.new(
          { 'PATH_INFO' => '/some/path/here', 'REQUEST_METHOD' => 'POST' }
        )
    ...
    it 'with group_name params it sends a list of snapshots of the given group' do
      ...
      struct2 =
        Rack::MiniProfiler::TimerStruct::Page.new(
          { 'PATH_INFO' => '/another/path/here', 'REQUEST_METHOD' => 'DELETE' }
        )
    ...
    it 'when snapshot is not found a 404 response is given' do
      ...
      expect(last_response.status).to eq(404)
      expect(last_response.body).to eq(
        "Snapshot with id 'nonsenseidhere' not found"
      )

      get "#{base_url}results?id=%22%3E%3Cqss%3E&snapshot=true"
      expect(last_response.status).to eq(404)
      expect(last_response.body).to eq(
        "Snapshot with id '&quot;&gt;&lt;qss&gt;' not found"
      ),
      'id should be escaped to prevent XSS'
    end
  end
end
