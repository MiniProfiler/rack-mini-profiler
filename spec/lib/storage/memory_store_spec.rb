...
describe Rack::MiniProfiler::MemoryStore do
  context 'page struct' do
    before { @store = Rack::MiniProfiler::MemoryStore.new }
    ...
    describe 'storage' do
      ...
      it 'can list unviewed items for a user' do
        ...
        expect(@store.get_unviewed_ids('a')).to eq(%w[XYZ ABC])
      end
  ...
  describe 'cleanup_cache' do
    ...
    it 'cleans up expired values' do
      old_page_struct = {
        ...
        random: 'random',
        started:
          ((Process.clock_gettime(Process::CLOCK_MONOTONIC) - 2) * 1000).to_i
      }
