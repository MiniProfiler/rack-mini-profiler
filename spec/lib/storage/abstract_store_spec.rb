...
describe Rack::MiniProfiler::AbstractStore do
  ...
  test_store =
    Class
      .new(Rack::MiniProfiler::AbstractStore) do
  ...
  describe '#snapshot_groups_overview' do
    ...
    it 'groups snapshots by request method and request path' do
      ...
      expect(groups.map { |g| g[:name] }).to eq(
        %w[
          DELETE\ users#delete
          POST\ /some/path
    ...
    it 'sorts groups from worst to best' do
      expect(groups.map { |g| g[:worst_score] }).to eq(
        [831.4232, 75.3793, 63.984, 50.314]
      )
    end

    it 'includes best_score' do
      expect(groups.map { |g| g[:best_score] }).to eq(
        [24.243, 75.3793, 15.424, 50.314]
      )
    end
  end
  ...
  describe '#find_snapshots_group' do
    ...
    it 'includes custom_fields with each snapshot' do
      ...
      expect(g3.map { |s| s[:custom_fields] }).to eq(
        [{ f4: '1' }, { f2: 'val' }]
      )
