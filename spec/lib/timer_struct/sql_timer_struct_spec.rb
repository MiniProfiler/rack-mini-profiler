...
describe Rack::MiniProfiler::TimerStruct::Sql do
  ...
  describe 'valid sql timer' do
    before do
      @sql =
        Rack::MiniProfiler::TimerStruct::Sql.new(
          'SELECT * FROM users',
          200,
          @page,
          nil
  ...
  describe 'backtrace' do
    ...
    it "doesn't include rspec if we filter for only app" do
      Rack::MiniProfiler.config.backtrace_includes = [%r{\/app}]
      sql =
        Rack::MiniProfiler::TimerStruct::Sql.new(
          'SELECT * FROM users',
          200,
    ...
    it 'includes rspec if we filter for it' do
      Rack::MiniProfiler.config.backtrace_includes = [%r{\/(app|rspec)}]
      sql =
        Rack::MiniProfiler::TimerStruct::Sql.new(
          'SELECT * FROM users',
          200,
    ...
    it 'ignores rspec if we specifically ignore it' do
      Rack::MiniProfiler.config.backtrace_ignores = [%r{\/rspec}]
      sql =
        Rack::MiniProfiler::TimerStruct::Sql.new(
          'SELECT * FROM users',
          200,
  ...
  describe '#params' do
    let(:sample_params) do
      [%w[name admin], ['value', 'string with more than 20'], ['limit', 1]]
    end
